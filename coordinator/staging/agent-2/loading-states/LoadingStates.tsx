      ...
      {Array.from({ length: lines }).map((_, index) => (
        <div
          ...
          className={cn(
            ...
            index === 0 && animated ? "animate-shimmer" : ""
          )}
          style={{
            backgroundImage:
              index === 0 && animated
                ? "linear-gradient(90deg, transparent, var(--shimmer-color), transparent)"
                : undefined,
          }}
        />
      ))}
  ...
  return (
    <div className={cn("flex space-x-[var(--spacing-sm)]", className)}>
      {Array.from({ length: count }).map((_, index) => (
        <div
          // biome-ignore lint/suspicious/noArrayIndexKey: Static animation dots won't reorder
          key={index}
  ...
  return (
    ...
    >
      <div className="text-center">
        ...
        <p className="text-[var(--fg-70)] text-[var(--font-size-sm)]">
...
export function SkeletonList({ lines = 3, className = "" }: SkeletonListProps) {
  return (
    <div className={cn("space-y-[var(--spacing-md)]", className)}>
      {Array.from({ length: lines }).map((_, index) => (
        // biome-ignore lint/suspicious/noArrayIndexKey: Static skeleton items won't reorder
        <div key={index} className="flex items-center space-x-[var(--spacing-md)]">
