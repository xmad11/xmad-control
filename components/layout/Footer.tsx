...
const socialLinks = [
  ...
  },
  {
    name: "X (Twitter)",
    icon: XIcon,
    href: "https://x.com/the.ss",
    followers: "Follow us",
  },
  {
    name: "Email",
    icon: EnvelopeIcon,
    href: "mailto:contact@shadi.ae",
    followers: "Contact us",
  },
]
...
const footerSections = [
  {
    title: "Platform",
    links: [
      { label: "About Shadi.ae", href: "/about" },
      { label: "How It Works", href: "/how-it-works" },
      { label: "For Restaurants", href: "/business" },
      { label: "For Home Businesses", href: "/business/home" },
  ...
  },
  {
    ...
    links: [
      ...
      { label: "Budget Friendly", href: "/budget" },
    ],
  },
  {
    title: "Business",
    links: [
      { label: "Business Sign In", href: "/business/signin" },
      { label: "Business Sign Up", href: "/business/signup" },
      { label: "Request a Visit", href: "/business/request-visit" },
      { label: "Send Products", href: "/business/send-products" },
...
const Footer = memo(function Footer() {
  ...
  const logoClasses = useMemo(() => {
    return `h-[var(--logo-size-footer)] opacity-50 ${mode === "dark" ? "invert" : ""}`
  }, [mode])
  ...
  return (
    <footer className="bg-[var(--fg-3)] border-t border-[var(--fg-10)]">
            ...
            {socialLinks.map((social) => (
              <a
                ...
                className={`
                  ...
                  w-[var(--icon-size-xl)] h-[var(--icon-size-xl)]
                  rounded-[var(--radius-lg)]
                  border border-[var(--fg-10)] bg-[var(--card-bg)]
      ...
      <div className="max-w-[var(--page-max-width)] mx-auto px-[var(--page-padding-x)] py-[var(--spacing-2xl)]">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-[var(--spacing-lg)] mb-[var(--spacing-2xl)]">
          {footerSections.map((section) => (
            <div key={section.title}>
              <h3 className="text-[var(--font-size-sm)] font-bold text-[var(--fg)] mb-[var(--spacing-md)]">
                {section.title}
              </h3>
              <ul className="space-y-[var(--spacing-xs)]">
                {section.links.map((link) => (
                  <li key={link.label}>
                    <a
                      href={link.href}
                      className="text-[var(--font-size-xs)] text-secondary-gray hover:text-[var(--color-primary)] transition-colors"
                    >
                      {link.label}
                    </a>
                  </li>
        ...
        {/* Bottom Bar */}
        <div className="pt-[var(--spacing-xl)] border-t border-[var(--fg-10)]">
