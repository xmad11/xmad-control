...
function CalendarWidget({ date, selectedDate, onDateSelect, className }: CalendarWidgetProps) {
...
function EventsCalendarWidget({ date, events = [], className }: EventsCalendarWidgetProps) {
  const [internalDate, setInternalDate] = React.useState<Date | null>(date ?? null)
  React.useEffect(() => {
    if (!date) setInternalDate(new Date())
  }, [date])
