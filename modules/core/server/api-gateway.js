// ~/xmad-control/modules/core/server/api-gateway.js
...
const QUEUE_FILE     = process.env.XMAD_QUEUE_FILE    || `${os.homedir()}/xmad-control/storage/automation/queue.json`;
...
const server = http.createServer(async (req, res) => {
  ...
  if (pth.startsWith("/api/automation/run/") && method === "POST") {
    ...
    if (!job) { sendError(res, 404, "Job not found"); return; }
    // Mark as running
    job.status = "running";
