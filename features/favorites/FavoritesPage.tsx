...
import { FavoritesEmptyState } from "./components/EmptyState"
import { FavoriteButton } from "./components/FavoriteButton"
import { FavoritesSortDropdown } from "./components/SortDropdown"
import type { BlogFavorite, FavoriteSortOption, FavoriteType, RestaurantFavorite } from "./types"
...
function FavoritesPageClient() {
  ...
  const { favorites, removeFavorite, clearAll, getFavoritesByType } = useFavorites()

  // Get current tab's favorites
  const favoriteType = activeTab === "restaurants" ? "restaurant" : "blog"
  const currentFavorites = getFavoritesByType(favoriteType, sortOption)
  ...
  const handleClearAll = useCallback(() => {
    clearAll(favoriteType)
    setShowConfirmDialog(false)
  }, [favoriteType, clearAll])
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-[var(--spacing-md)]">
                {currentFavorites.map((item) => {
                  const restaurant = item as RestaurantFavorite
                  return (
                    <div key={item.id} className="relative group">
                      ...
                      <RestaurantCard
                        ...
                        price={(restaurant.priceRange || "$$") as "$" | "$$" | "$$$" | "$$$$"}
                      ...
                      <div className="absolute top-[var(--spacing-sm)] left-[var(--spacing-sm)]">
                        <FavoriteButton
                          isFavorite={true}
                          onToggle={() => handleRemoveFavorite(restaurant.restaurantId)}
                          size="sm"
                          iconOnly
                        />
                      </div>
          ...
          {hasFavorites && activeTab === "blogs" && (
            <>
              ...
              <div className="grid grid-cols-1 gap-[var(--spacing-md)] sm:grid-cols-2 lg:grid-cols-3">
                {currentFavorites.map((item) => {
                  const blog = item as BlogFavorite
                  return (
                    <div
                      key={item.id}
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => handleRemoveFavorite(blog.blogId)}
                        className="absolute top-[var(--spacing-sm)] right-[var(--spacing-sm)] p-[var(--spacing-xs)] rounded-full bg-[var(--color-error)] text-white opacity-0 group-hover:opacity-100 transition-opacity duration-[var(--duration-fast)] hover:scale-110"
                        aria-label="Remove from favorites"
                      >
                        <TrashIcon className="h-[var(--icon-size-sm)] w-[var(--icon-size-sm)]" />
                      ...
                      <div className="absolute top-[var(--spacing-sm)] left-[var(--spacing-sm)]">
                        <FavoriteButton
                          isFavorite={true}
                          onToggle={() => handleRemoveFavorite(blog.blogId)}
                          size="sm"
                          iconOnly
                        />
                      </div>
...
export default memo(FavoritesPageClient)
